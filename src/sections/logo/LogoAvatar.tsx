          ...
          {avatarBgs.map(a => (
            <div key={a.label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
              <div style={{ width: "100%", aspectRatio: "1", borderRadius: "50%", background: a.bg, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                <StackedLogoSvg markFill={a.mark} wordmarkFill={a.wm} innerTextFill={a.inner} style={{ width: "76%", height: "auto" }} />
              </div>
              <div style={{ fontSize: 11, color: "#111", letterSpacing: "0.04em", fontFamily: `var(--body-font, 'Inter'), sans-serif` }}>{a.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ borderTop: "1px solid #E5E5E5", paddingTop: 48, marginBottom: 80 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, maxWidth: 760 }}>
          {faviconVariants.map(v => (
            <div key={v.key}>
              <div style={{ fontSize: 11, color: "#111", letterSpacing: "0.04em", marginBottom: 16, fontFamily: `var(--body-font, 'Inter'), sans-serif` }}>{v.label}</div>
              <div style={{ display: "flex", alignItems: "flex-end", gap: 28 }}>
                {faviconSizes.map(size => (
                  <div key={size} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ fontSize: 10, color: "#111", fontFamily: `var(--body-font, 'Inter'), sans-serif` }}>{size}px</div>
                  </div>
                ))}
              </div>
            </div>
